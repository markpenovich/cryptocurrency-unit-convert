...
Units.convertBTC = function(value, from, to) {
  ...
  return new BigNumber(value, 10).times(btcUnits[from]).div(btcUnits[to]).round(4, BigNumber.ROUND_DOWN).toString(10);
}
...
Units.convertETH = function(value, from, to) {
  ...
  return new BigNumber(value, 10).times(ethUnits[from]).div(ethUnits[to]).round(4, BigNumber.ROUND_DOWN).toString(10);
}
...
Units.convertXRP = function(value, from, to) {
  ...
  return new BigNumber(value).times(xrpUnits[from]).div(xrpUnits[to]).round(3, BigNumber.ROUND_DOWN).toString(10);
}
...
Units.convertLTC = function(value, from, to) {
  ...
  return new BigNumber(value, 10).times(ltcUnits[from]).div(ltcUnits[to]).round(3, BigNumber.ROUND_DOWN).toString(10);
}
